...
export type DocRead = {
  ...
  // ── A 1040 CARRIES OBLIGATIONS AS WELL AS INCOME ──────────────────────────────────────────
  // Ricardo Barron, 2026-09-06: $18,000/yr of alimony sat on Schedule 1 line 19a of BOTH his
  // returns and reached no part of the file. It was only ever visible because the reader had
  // mentioned it in a free-text note; there was no field for it, so nothing could look for it.
  // $1,500/mo of undisclosed obligation on a live application.
  alimonyPaidAnnual?: number | null;      // Sch 1 line 19a — money PAID OUT, never income
  alimonyAgreementDate?: string | null;   // Sch 1 line 19c — the term is what decides if it counts
  // A ROLLOVER IS NOT INCOME. Same file: 1040 line 5a showed $109,320 gross against $7,686
  // taxable with the line-5c Rollover box CHECKED — ~$101,634 moved between retirement accounts.
  // The QC reviewer read the gross and called it "recurring distributions with presumed
  // continuance", one click away from $9,110/mo of invented income.
  retirementRolloverChecked?: boolean | null;  // 1040 line 4c or 5c "Rollover" box
...
export function toDocFact(r: DocRead): DocFact {
  return {
    ...
    alimonyPaidAnnual: r.alimonyPaidAnnual ?? null,
    alimonyAgreementDate: r.alimonyAgreementDate ?? null,
    retirementRolloverChecked: typeof r.retirementRolloverChecked === "boolean" ? r.retirementRolloverChecked : null,
    pensionGrossAnnual: r.pensionGrossAnnual ?? null,
    pensionTaxableAnnual: r.pensionTaxableAnnual ?? null,
    iraGrossAnnual: r.iraGrossAnnual ?? null,
    iraTaxableAnnual: r.iraTaxableAnnual ?? null,
