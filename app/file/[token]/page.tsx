        ...
        <div className="mt-6">
          ...
          <div className="space-y-2 mt-4">
            {docs.map((d) => {
              ...
              return (
                <div key={d.id} className="flex items-center justify-between bg-white border border-slate-200 rounded-xl px-4 py-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      {got ? <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" /> : <FileText className="w-4 h-4 text-slate-400 shrink-0" />}
                      <span className="font-medium truncate">{d.name}</span>
                      {d.required && !got && <span className="text-[10px] text-amber-400/80 shrink-0">required</span>}
                    </div>
                    {got && <div className="text-xs text-slate-400 mt-0.5 ml-6">{d.status === "accepted" ? "Accepted ✓" : d.status === "rejected" ? "Needs another copy" : "Received — under review"}{d.file_name ? ` · ${d.file_name}` : ""}</div>}
                  </div>
                  <div className="shrink-0 ml-3">
                    ...
                    <button onClick={() => fileInputs.current[d.id]?.click()} disabled={busyDoc === d.id}
                      className={`text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 ${got ? "bg-slate-100 hover:bg-slate-200 text-slate-600" : "bg-emerald-600 hover:bg-emerald-500 text-white font-semibold"}`}>
          ...
          <div className="mt-4">
            ...
            <button onClick={() => fileInputs.current["new"]?.click()} disabled={busyDoc === "new"}
              className="w-full border border-dashed border-slate-200 hover:border-emerald-300 text-slate-500 rounded-xl py-3 text-sm flex items-center justify-center gap-2">
