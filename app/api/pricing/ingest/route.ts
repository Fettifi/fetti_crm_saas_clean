...
import { addProducts, type PricingProduct } from "@/lib/pricing/compare";
import crypto from "crypto";

// AI rate-sheet ingestion. Upload a wholesaler's rate sheet (PDF/image) and
// Claude parses it into normalized pricing products. Auth-gated via /api/pricing.
//   POST /api/pricing/ingest   FormData { lenderName, doc }
...
Rules: capture one row per product/rate where you can read a rate and price. Pick the most representative lock period if several. INCLUDE ONLY fields you can read; omit the rest. If it's not a rate sheet, return []. Never invent numbers.`;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const products: PricingProduct[] = rows.map((r) => ({
      id: crypto.randomUUID(), lenderId, lenderName, uploadedAt: now,
    ...
    return NextResponse.json({ ok: true, lenderId, added: products.length });
  } catch (e: any) {
